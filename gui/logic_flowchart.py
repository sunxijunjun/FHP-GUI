...
def create_complex_flowchart():
    ...
    # 要求保持治理坐姿
    dot.node('B', 'Seat upright for 10 s', shape='rectangle')  # 矩形表示过程或操作步骤

    # 输入节点
    dot.node('C', 'Input: Sensor 2, Sensor 4, Face Data', shape='parallelogram')  # 平行四边形表示输入/输出操作

    # 判断节点1: 判断Sensor是否都在400-1200范围内
    dot.node('D', '400 (too close) < All Sensor < 1200 (sensor miss-focus)? ', shape='diamond')  # 菱形表示判断或决策点
    ...
    dot.node('G2', '400 (too close) < All Sensor < 1200 (sensor miss-focus)? ', shape='diamond')

    # 计算差值和角度
    dot.node('H', 'Calculate Difference (Sensor 4 - Sensor 2)\nCalculate Head Tilt Angle\nModel Prediction', shape='rectangle')

    # 判断节点3: 头部倾斜角度是否大于20度
    dot.node('I', 'Head Tilt > 20°?', shape='diamond')

    # 判断节点4: Sensor 4和Sensor 2的差值是否超过阈值
    dot.node('J', 'Model Output?', shape='diamond')
    ...
    dot.edge('J', 'L', label='Abnormal')
    dot.edge('J', 'R', label='Normal')  # 这里改为连接Restart节点
    dot.edge('K', 'M')
    dot.edge('L', 'M')
